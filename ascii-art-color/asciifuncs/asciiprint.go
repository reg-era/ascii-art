...
func AsciiProcess(word, color, spcolor string) string {
	res := ""
	if len(word) == 0 {
		return res
	}
	...
	if len(word) != 1 && CheckNewLine(word) {
		...
		for i := 0; i < len(tabword); i++ {
			if (len(tabword[i]) == 0) || (len(tabword[i]) == 0 && i != len(tabword)-1 && len(tabword[i+1]) == 0) {
				...
				tab := []rune(tabword[i])
				res += AsciiPrint(tab, spcolor, SearchColor(color))
			}
		}
	}
	return res
...
func AsciiPrint(word []rune, spcolor, color string) string {
	res := ""
	if len(word) == 0 {
		return "\n"
	}
	if spcolor != "" {
		for i := 0; i < 8; i++ {
			for j := 0; j < len(word); j++ {
				...
				line := ""
				if strings.Contains(spcolor, string(word[j])) {
					line = color + AsciiMap[word[j]][i] + "\033[0m"
				} else {
